# -*- coding: utf-8 -*-
# @Time    : 2024/11/28 14:14
# @Author  : Galleons
# @File    : position_import.py
...
def create_point(publish_id: int, doc: Dict[str, Any]) -> models.PointStruct:
    ...
    try:
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            future_name = executor.submit(get_embedding, doc['digital_desc'], 1)
            future_desc = executor.submit(get_embedding, doc['position_name'], 2)
            future_req = executor.submit(get_embedding, doc['description'], 1)

            return models.PointStruct(
                id=publish_id,
                vector={
                    'digital_desc': future_name.result(),
                    'position_name': future_desc.result(),
                    'description': future_req.result(),
                },
                payload=doc
            )
    except Exception as e:
...
def main():
    try:
        # 读取数据
        job_names = pd.read_csv("/home/weyon2/DATA/table_data/jobs/云生涯岗位信息.csv")
        job_names.dropna(subset=['position_name', 'digital_desc', 'description'], inplace=True)
        ...
        for doc in tqdm(json_job, total=len(json_job), desc="Creating points"):
            try:
                point = create_point(doc['position_id'], doc)
                points.append(point)
            except Exception as e:
