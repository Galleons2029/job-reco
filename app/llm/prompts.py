...
class Prompts(BaseSettings):
    ...
    job_item_fill: str = """你是一个专业的招聘专家，你对{company}这个公司的{position}这个岗位有很深的理解，这个公司的信息是{companyIntro}，职位要求是{positionIntro}。
        ...
        你需要根据上述基本信息，准确的生成下面示例格式的内容并用JSON格式返回：
        {
          ...
          "工作性质": "校招/社招/实习只能选一个",
          "学历要求": "不限/中专及以上/大专及以上/本科及以上/硕士及以上/博士及以上只能选一个",
    ...
    job_item_write: str = """你是一个专业的招聘专家，你对{company}这个公司的{position}这个岗位有很深的理解，这个公司的信息是{companyIntro}，职位要求是{positionIntro}。
    
        ## 任务要求
        你需要根据上述基本信息，准确的生成下面示例格式的内容,并用JSON格式返回：
        {
